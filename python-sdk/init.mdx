...
<ParamField path="production_monitoring" type="boolean" optional default="False">
  Flag to enable production monitoring mode for tracking sessions in production environments. When enabled, sessions appear in the Production Monitoring dashboard with comprehensive metrics on cost, performance, and errors. Can also be set via `LUCIDIC_PRODUCTION_MONITORING` environment variable. See [Production Monitoring](/features/production-monitoring) for details.
</ParamField>
...
<ParamField path="dataset_item_id" type="string" optional>
  ID of a dataset item being tested in this session. Links the session to a specific test case from a dataset. Use when running dataset tests to track which test case each session corresponds to. See [Working with Datasets](/python-sdk/datasets) for details.
</ParamField>
...
dataset_items = lai.get_dataset_items(dataset_id="test_suite_v1")

for item in dataset_items:
    session_id = lai.init(
        session_name=f"Test: {item['name']}",
        dataset_item_id=item['id'],  # Links to specific test case
        rubrics=["correctness", "completeness"]
    )
