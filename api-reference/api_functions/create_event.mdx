...
<ParamField path="screenshots" type="List[str]" optional>
  List of base64-encoded screenshots associated with this event.
</ParamField>
...
screenshots = process_document_images(document_images)
lai.create_event(
    description="Extracted and analyzed document images",
    result=f"Processed {len(screenshots)} images",
...
- [`update_event`](/api-reference/api_functions/update_event) - Update an existing event
- [`end_event`](/api-reference/api_functions/end_event) - End an event
- [`create_step`](/api-reference/api_functions/create_step) - Create a step to contain events