...
### `uploadImage(imageData: Buffer | string): Promise<string>`

Upload an image to Lucidic storage.

```typescript
// Upload buffer
const buffer = fs.readFileSync('image.png');
const url1 = await lai.uploadImage(buffer);

// Upload base64
const base64 = 'iVBORw0KGgoAAAA...';
const url2 = await lai.uploadImage(base64);
```

### `setMaskingFunction(fn: (text: string) => string): void`

Set a custom data masking function.
...
### `withStorageContext<T>(fn: T): T`

Wrap a function to enable multimodal content tracking.

```typescript
const trackedFunction = lai.withStorageContext(async () => {
  // Images processed here will be tracked
