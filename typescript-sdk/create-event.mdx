...
}): Promise<{ eventId: string; stepId: string }>
```
...
- `stepId` - Step the event is associated with
...
```typescript
// Create event with visual context (pass data URLs)
await lai.createEvent({
